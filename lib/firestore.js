...
class Firestore {
	...
	if (admin.apps.length === 0) {
			admin.initializeApp({
				credential: admin.credential.cert( dataSourceProps.serviceAccount )
			});
			admin.firestore().settings({ timestampsInSnapshots: true })
		}
		this.db = admin.firestore().collection(dataSourceProps.project).doc(dataSourceProps.environment);
	}
	...
	async update(model, where, data, options, callback) {
		console.log(where);
		if (where) {
			const keyId = Object.keys(where)[0];
		const snapshots = await this.db.collection(model).where(keyId, '==', where[keyId]).get();
		let docId;
		...
		if (docId) {
			await this.db.collection(model).doc(docId).update(data).then((res) => {
					return res;
			});
			callback(null, []);
		} else {
			callback(`User not exist ${JSON.stringify(data)}`);
		}

	} else {
		callback(`Property id does not have on model ${model}`);
	}
	}
     ...
     * @param [callback] The callback function
     */
	async destroyAll(model, where, callback) {
		console.log(where);
		if (where) {
			const keyId = Object.keys(where)[0];
		const snapshots = await this.db.collection(model).where(keyId, '==', where[keyId]).get();
		const modelForDeletion = this.db;
		snapshots.forEach(snapshot => {
				console.log(snapshot);
				const docId = snapshot.ref.id
				console.log(docId);
			 modelForDeletion.collection(model).doc(docId).delete();
		});
		callback(null, []);
	} else {
		callback(`Properte id does not have on model ${model}`);
	}
		// const self = this;
		// if (where.id) {
		// 	this.exists(model, where.id, null, (err, res) => {
	...
	buildQuery(filter, query, callback) {
		...
		if (filter.where && filter.where.id) {
			...
			this.buildWhereQuery(filter.where, query, newQuery => {
				if (filter.order) {
					newQuery = newQuery.orderBy(filter.order, 'desc');
				}
				if (filter.limit) {
					newQuery = newQuery.limit(filter.limit);
				}
				callback(newQuery);
			});
		} else {
			if (filter.order) {
				response = response.orderBy(filter.order, 'desc');
			}
			if (filter.limit) {
				response = response.limit(filter.limit);
			}
			callback(response);
		}
	}
